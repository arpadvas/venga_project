...
.controller('cragsCtrl', function(Crag, $scope) {
	...
	app.search = function(searchByCragName, searchByCragCountry) {
		if (searchByCragName || searchByCragCountry) {
			app.loading = true;
			$scope.keywordData = {};
			if (searchByCragName) {
				$scope.keywordData.nameKeyword = searchByCragName;
			} else {
				$scope.keywordData.nameKeyword = '';
			}
			if (searchByCragCountry) {
				$scope.keywordData.countryKeyword = searchByCragCountry;
			} else {
				$scope.keywordData.countryKeyword = '';
			}
			Crag.getCrags($scope.keywordData).then(function(data) {
	...
	app.clear = function() {
		app.crags = undefined;
		$scope.searchByCragName = undefined; 
		$scope.searchByCragCountry = undefined;
